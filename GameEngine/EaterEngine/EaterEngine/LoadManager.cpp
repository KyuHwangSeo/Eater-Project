...
void LoadManager::LoadQuad()
{
	ModelData* SaveMesh = new ModelData();

	LoadMeshData* TopMesh = CreateQuadObject();
	SaveMesh->TopMeshList.push_back(TopMesh);

	ModelList.insert({ "Quad" ,SaveMesh });
}
...
LoadMeshData* LoadManager::CreateTerrainObject(ParserData::Mesh* mesh, std::string maskName)
{
	...
	terrain->Mask_Name = maskName;

	SetData(terrain, mesh);

	//버퍼값 읽어오기
	GEngine->CreateMeshBuffer(mesh, terrain);
...
LoadMeshData* LoadManager::CreateMeshObject(ParserData::Mesh* mesh)
{
	...
	//버퍼값 읽어오기
	GEngine->CreateMeshBuffer(mesh, model);
...
LoadMeshData* LoadManager::CreateQuadObject()
{
	//계층정보 받기
	...
	//버퍼값 읽어오기
	GEngine->CreateMeshBuffer(nullptr, quad);
...
void LoadManager::SetData(LoadMeshData* MeshData, ParserData::Mesh* LoadData)
{
	...
	//스플릿 안한 데이터를 받는다 받기전 로컬행렬로 버텍스의 위치를 이동시켜준다
	int VertexSize = (int)LoadData->m_OriginVertexList.size();
	int IndexSize = (int)LoadData->m_OriginIndexList.size();
	MeshData->m_OriginVertexList = new Vector3[VertexSize];
	MeshData->m_OriginIndexList = new UINT[IndexSize];

	MeshData->m_OriginVertexListCount = VertexSize;
	MeshData->m_OriginIndexListCount = IndexSize;

	//버텍스의 Vecter와 로컬행렬을 한번 곱해준다
	for (int i = 0; i < VertexSize; i++)
	{
		...
		MeshData->m_OriginVertexList[i] = WorldVertex;
	}

	//인덱스의 데이터를 받아온다
	for (int i = 0; i < IndexSize; i++)
	{
		MeshData->m_OriginIndexList[i] = LoadData->m_OriginIndexList[i];
	}
}
