...
void NetworkManager::AddPlayer(GameObject* mClient01, GameObject* mClient02)
{
	//객체를 받는다
...
void NetworkManager::SetMovePos(Vector3 Pos)
{
	DebugManager::Print("Client가 움직인다");
}

void NetworkManager::C2S_KEEP_ALIVE_CHECK_REQ()
{
	///플레이어가 연결되었는지 보냄
	DebugManager::Print("Client-->-->-->-->--LINK-->-->-->-->Server");
}

void NetworkManager::C2S_KEEP_ALIVE_CHECK_RES()
{
	///플레이어가 연결되었는지 받음
	DebugManager::Print("Client--<--<--<--<--LINK--<--<--<--<Server");
}

void NetworkManager::S2C_START_GAME()
{
	///게임시작에 필요한 데이터를 받는다
...
void NetworkManager::C2S_PLAYER_MOVE_REQ(Vector3 Pos, Vector3 Direction, float Speed)
{
	///서버로 데이터를 넘긴다
	DebugManager::Print("Client-->-->-->-->--Move-->-->-->-->Server");
}

void NetworkManager::C2S_PLAYER_MOVE_RES()
{
	///서버에서 데이터를 받는다
